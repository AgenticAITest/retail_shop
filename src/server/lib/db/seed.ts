...
async function seedTestTenant() {
  ...
  await tenantDb.transaction(async (tx) => {
    ...
    const allPerms = await tx.insert(tenantSchema.permission).values([
      ...
      { id: crypto.randomUUID(), code: 'retail.grn.transition',       name: 'Transition GRN',       description: 'Change GRN status' },
      ...
      { id: crypto.randomUUID(), code: 'retail.transfer.transition',  name: 'Transition Transfer',  description: 'Change transfer status' },
      ...
      { id: crypto.randomUUID(), code: 'retail.inventory.alerts',     name: 'Manage Stock Alerts',  description: 'Configure stock alert thresholds' },
