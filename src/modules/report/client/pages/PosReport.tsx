        ...
        <div className="flex items-center gap-2">
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end">
              <DropdownMenuItem onClick={() => exportToCsv(`pos-cashier-${period}d`, ['Cashier', 'Transactions', 'Revenue (IDR)', 'Avg Basket (IDR)'], cashiers.map(c => [c.cashier_name, c.transactions, c.revenue, c.avg_basket]))}>Export CSV</DropdownMenuItem>
              <DropdownMenuItem onClick={() => exportToXlsx(`pos-cashier-${period}d`, 'Cashier Performance', ['Cashier', 'Transactions', 'Revenue (IDR)', 'Avg Basket (IDR)'], cashiers.map(c => [c.cashier_name, c.transactions, c.revenue, c.avg_basket]))}>Export Excel (XLSX)</DropdownMenuItem>
              <DropdownMenuItem onClick={() => exportToPdf(`pos-cashier-${period}d`, `POS Report — Cashier Performance (Last ${period} days)`, ['Cashier', 'Transactions', 'Revenue (IDR)', 'Avg Basket (IDR)'], cashiers.map(c => [c.cashier_name, c.transactions, c.revenue, c.avg_basket]))}>Export PDF</DropdownMenuItem>
            </DropdownMenuContent>
          </DropdownMenu>
        </div>
      </header>
