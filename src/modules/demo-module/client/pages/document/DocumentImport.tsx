...
import { documentImportSchema } from './documentFormSchema';
...
const DocumentImport = () => {
  ...
  const form = useForm<z.infer<typeof documentImportSchema>>({
    resolver: zodResolver(documentImportSchema),
    mode: "onSubmit",
    reValidateMode: "onSubmit"
  });

  function onSubmit(values: z.infer<typeof documentImportSchema>) {
    //console.log(values);
    setIsLoading(true);
    const formData = new FormData();
    formData.append("file", values.file);
    axios
      .post("/api/modules/demo-module/document/import", formData, {
        headers: {
      ...
      .then(() => {
        ...
        toast.success("Documents imported successfully.", { duration: 8000 });
      })
      .catch((error) => {
        console.error("Error to import documents :", error);
                ...
                render={({ field: { value, onChange, ...fieldProps } }) => (
                  <FormItem className="grid grid-cols-12 gap-2 items-start">
                    ...
                    <div className="col-span-12 md:col-span-10 space-y-2">
                      ...
                      <FormMessage />
                      <a href="/samples/document_template.csv" className="flex gap-2 pt-4 text-sm hover:underline">
        ...
        <AlertDialogContent>
          <AlertDialogHeader className='flex w-full items-center text-center'>
            ...
            <AlertDialogDescription>
              Exporting documents may take a while.
            </AlertDialogDescription>
          </AlertDialogHeader>
        </AlertDialogContent>
    </AlertDialog>
...
export default withModuleAuthorization(DocumentImport, {
