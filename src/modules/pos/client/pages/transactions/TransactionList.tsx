...
const TransactionList = () => {
  ...
  useEffect(() => {
    if (loading) {
      axios.get('/api/modules/pos/transaction', { params: { page, perPage, sort, order, filter, status: statusFilter } })
        .then(response => { setTransactions(response.data.transactions || []); setCount(response.data.count || 0); })
      ...
      <div className="@container/main flex flex-1 flex-col gap-2">
        <div className="flex flex-col gap-4 px-2 py-2 md:gap-6">
          <div className="flex gap-2 flex-wrap">
            <Button onClick={() => navigate('/pos')} variant="default">
              Open POS Terminal
            </Button>
            <div className="ml-auto flex items-center gap-2">
              <Select value={statusFilter} onValueChange={setStatusFilter}>
                <SelectTrigger className="w-[160px] h-8"><SelectValue placeholder="Filter by status" /></SelectTrigger>
                <SelectContent>
                  {STATUS_OPTIONS.map(opt => (<SelectItem key={opt.value} value={opt.value}>{opt.label}</SelectItem>))}
                </SelectContent>
              </Select>
              <InputGroup>
                <DebouncedInput onChange={value => setFilter(String(value))} placeholder="Search transaction ID..." type="text" value={filter ?? ''} />
            ...
            <Table>
              <TableHeader className="bg-muted/20 font-semibold">
                <TableRow>
                  ...
                  <TableHead className="w-[200px] py-2"><SortButton column="transactionId" label="Transaction ID" sort={sort} order={order} sortBy={sortBy} /></TableHead>
                  <TableHead className="w-[140px] py-2">Location</TableHead>
                  <TableHead className="w-[140px] py-2">Cashier</TableHead>
                  <TableHead className="w-[120px] py-2 text-right"><SortButton column="totalAmount" label="Total" sort={sort} order={order} sortBy={sortBy} /></TableHead>
                  <TableHead className="w-[100px] py-2">Payment</TableHead>
                  <TableHead className="w-[100px] py-2"><SortButton column="status" label="Status" sort={sort} order={order} sortBy={sortBy} /></TableHead>
                  <TableHead className="w-[160px] py-2"><SortButton column="createdAt" label="Date" sort={sort} order={order} sortBy={sortBy} /></TableHead>
                ...
                {transactions.map((txn, i) => (
                  <TableRow key={txn.id}>
                    ...
                    <TableCell className="font-medium font-mono text-sm">
                      <Link to={`/console/modules/pos/transaction/${txn.id}`} className="no-underline hover:underline">{txn.transactionId}</Link>
                    </TableCell>
                    <TableCell>{txn.locationName || '-'}</TableCell>
                    <TableCell>{txn.cashierName || '-'}</TableCell>
                    <TableCell className="text-right font-medium">{formatCurrency(txn.totalAmount)}</TableCell>
                    <TableCell className="capitalize">{txn.paymentMethod || '-'}</TableCell>
                    <TableCell>
                      <span className={`inline-flex items-center rounded-full px-2 py-1 text-xs font-medium capitalize ${STATUS_COLORS[txn.status] || ''}`}>
                        {txn.status}
                      </span>
                    </TableCell>
                    <TableCell className="text-sm">{formatDate(txn.completedAt || txn.createdAt)}</TableCell>
                    <TableCell>
                      <Tooltip><TooltipTrigger asChild>
                        <Button variant="secondary" size="sm" onClick={() => navigate(`/console/modules/pos/transaction/${txn.id}`)}><Eye size={16} /></Button>
...
export default withModuleAuthorization(TransactionList, { moduleId: 'pos', moduleName: 'Point of Sale' });
