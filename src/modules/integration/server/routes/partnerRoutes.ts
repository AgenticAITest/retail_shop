...
import { partner } from '../lib/db/schemas/integration';
import { partnerEditInputSchema, partnerInputSchema, partnerQuerySchema } from '../schemas/partnerSchema';

const partnerRoutes = Router();

partnerRoutes.use(resolveTenantContext());
partnerRoutes.use(authenticated());
partnerRoutes.use(checkModuleAuthorization('integration'));
 ...
 */
partnerRoutes.get("/", authorized('ADMIN', 'integration.partner.view'), async (req, res) => {
  ...
  try {
    ...
    const partners = await req.tenantDb
      .select()
      .from(partner)
 ...
 *         description: Forbidden
 */
partnerRoutes.post("/", validateData(partnerInputSchema), authorized('ADMIN', 'integration.partner.create'), async (req, res) => {
  if (!req.tenantDb) {
    return res.status(500).json({ error: "Tenant database connection not found." });
  }
  try {
    ...
    const newPartner = await req.tenantDb
      .insert(partner)
      .values({
 ...
 *         description: Forbidden
 */
partnerRoutes.get("/:id", authorized('ADMIN', 'integration.partner.view'), async (req, res) => {
  if (!req.tenantDb) {
    return res.status(500).json({ error: "Tenant database connection not found." });
  }
  try {
    ...
    const foundPartner = await req.tenantDb
      .select()
      .from(partner)
      .where(and(
        eq(partner.id, id)
 ...
 *         description: Forbidden
 */
partnerRoutes.put("/:id", validateData(partnerEditInputSchema), authorized('ADMIN', 'integration.partner.edit'), async (req, res) => {
  if (!req.tenantDb) {
    return res.status(500).json({ error: "Tenant database connection not found." });
  }
  ...
  try {
    ...
    const existingPartner = await req.tenantDb
      .select()
      .from(partner)
      .where(and(
        eq(partner.id, id)
    ...
    const updatedPartner = await req.tenantDb
      .update(partner)
      .set({
        code,
        name,
 ...
 *         description: Forbidden
 */
partnerRoutes.delete("/:id", authorized('ADMIN', 'integration.partner.delete'), async (req, res) => {
  if (!req.tenantDb) {
    return res.status(500).json({ error: "Tenant database connection not found." });
  }
  try {
