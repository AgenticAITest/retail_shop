...
const EventDetail = () => {
  ...
  const [webhookEvent, setWebhookEvent] = useState<Event | null>(null);
  ...
  useEffect(() => {
    if (id && isAuthorized(["ADMIN"], ['integration.event.view'])) {
      loadWebhookEvent();
    } else if (!isAuthorized(["ADMIN"], ['integration.event.view'])) {
      navigate('/console/modules/integration/event');
    }
  }, [id, isAuthorized]);
      ...
      <div className="mx-2 space-y-6">
        <Card>
          ...
          <CardContent>
            ...
            <Button onClick={() => navigate('/console/master/event')} className="mt-4">
              Back to Events
            </Button>
          </CardContent>
        </Card>
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          {isAuthorized(["ADMIN"], ['integration.event.edit']) && (
            <Button onClick={() => navigate(`/console/modules/integration/event/${id}/edit`)}>
...
export default withModuleAuthorization(EventDetail, {
